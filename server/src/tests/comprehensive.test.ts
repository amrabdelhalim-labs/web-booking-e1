...
async function runTests(): Promise<void> {
  ...
  assert(event1BookingCount === 2, `Event1 has 2 bookings (got ${event1BookingCount})`);
  ...
  assert(updatedEvent!.title === 'مؤتمر التقنية السنوي (محدث)', 'Title updated correctly');
  ...
  assert(remainingBookings === 2, `2 bookings remaining (got ${remainingBookings})`);
  ...
  assert(bookingsAfter === 1, `1 booking remaining (got ${bookingsAfter})`);
