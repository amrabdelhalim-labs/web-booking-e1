...
async function runTests(): Promise<void> {
  ...
  assert(
    health.repositories.booking === true,
    "Booking repository is accessible"
  );
  ...
  assert(
    allEvents[0].creator !== undefined,
    "Event has populated creator field"
  );
  ...
  assert(
    userEvents.length === 2,
    `Found 2 events for user1 (got ${userEvents.length})`
  );
  ...
  assert(
    searchResults[0].title === "مؤتمر التقنية",
  ...
  assert(
    searchAll.length >= 2,
    `Search 'ال' returns multiple results (got ${searchAll.length})`
  );
  ...
  const eventIds = await repos.event.getEventIdsByCreator(
    user1._id.toString()
  );
  assert(
    eventIds.length === 2,
    `Got 2 event IDs for user1 (got ${eventIds.length})`
  );
  ...
  const hasBooked = await repos.booking.userHasBooked(
    user2._id.toString(),
    event1._id.toString()
  );
  ...
  assert(
    ...
    `Found 2 bookings for user2 (got ${userBookings.length})`
  );

  // Find by ID with details
  logStep("Finding booking by ID with details...");
  const foundBooking = await repos.booking.findByIdWithDetails(
  ...
  const deletedByEvent = await repos.booking.deleteByEvent(
    event1._id.toString()
  );
  ...
  assert(
    remainingBookings === 1,
    `1 booking remaining (got ${remainingBookings})`
  );
  ...
  assert(
    eventsAfter === 1,
    `1 event remaining after cascade (got ${eventsAfter})`
  );

  const bookingsAfter = await repos.booking.count();
  assert(
    bookingsAfter === 0,
    `0 bookings remaining after cascade (got ${bookingsAfter})`
  );
