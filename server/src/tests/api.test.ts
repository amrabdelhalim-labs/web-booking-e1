...
const TEST_DB_URL =
  ...
  config.dbUrl.replace(/\/[^/]+$/, "/event-booking-test-api");
const API_PORT = 4001;
...
async function runTests(): Promise<void> {
  ...
  try {
    ...
    assert(
      register1.data?.createUser?.token !== undefined,
      "Registration returns token"
    );
    assert(
      register1.data?.createUser?.username === "عمرو",
    ...
    assert(
      login.data?.login?.userId === userId,
      "Login returns correct userId"
    );
    ...
    `);
    assert(
      noAuth.errors !== undefined && noAuth.errors.length > 0,
      "Bookings query requires auth"
    );
    assert(
    ...
    `);
    assert(
      noAuthEvent.errors !== undefined,
      "Creating event requires auth"
    );
    ...
    assert(
      createEvent.data?.createEvent?.title === "مؤتمر الاختبار",
      "Event has correct title"
    );
    assert(
      createEvent.data?.createEvent?.price === 150,
      "Event has correct price"
    );
    assert(
      createEvent.data?.createEvent?.creator?._id === userId,
      "Event creator is correct user"
    );
    ...
    assert(
      dupTitle.errors !== undefined,
      "Duplicate event title returns error"
    );
    ...
    assert(
      queryEvents.data?.events?.length >= 1,
    ...
    assert(
      userEvents.data?.getUserEvents?.length === 1,
      "User has 1 event"
    );
    ...
    assert(
      updateEvent.data?.updateEvent?.price === 200,
      "Event price updated"
    );
    ...
    assert(
      nonOwnerUpdate.errors !== undefined,
    ...
    assert(
      dupBooking.errors !== undefined,
      "Duplicate booking returns error"
    );
    ...
    assert(
      bookings.data?.bookings?.length === 1,
      "User has 1 booking"
    );
    ...
    assert(
      cancelBooking.data?.cancelBooking?.title !== undefined,
      "Cancel returns event data"
    );
    ...
    assert(
      updateUser.data?.updateUser?.username === "عمرو المحدث",
    ...
    assert(
      errorResp.errors?.[0]?.message !== undefined,
      "Error has message"
    );
    assert(
      errorResp.errors?.[0]?.extensions?.code !== undefined,
      "Error has extension code"
    );
