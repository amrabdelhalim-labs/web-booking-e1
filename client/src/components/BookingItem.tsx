...
export default function BookingItem({ _id, event, createdAt, onCancelBooking }: BookingItemProps) {
  return (
    <li className="bookings-item">
      <div className="booking-info">
        <strong>{event.title}</strong>
        <span className="booking-meta">
          {event.creator?.username} · ${event.price} · {formatDateShort(event.date)}
        </span>
        <small className="booking-date">تاريخ الحجز: {formatDateArabic(createdAt)}</small>
      </div>
      <button className="btn btn-danger btn-sm" onClick={() => onCancelBooking(_id)}>
        إلغاء
      </button>
    </li>
  );
