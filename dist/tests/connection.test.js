...
async function testMongoDBConnection() {
    ...
    console.log(`   Connection String: ${config_1.config.dbUrl.replace(/\/\/.*@/, '//<credentials>@')}`);
    console.log(`   Port: ${config_1.config.port}`);
    console.log(`   Node Env: ${process.env.NODE_ENV || 'development'}`);
    if (!config_1.config.dbUrl || config_1.config.dbUrl === 'mongodb://localhost:27017/event-booking') {
    ...
    else if (config_1.config.dbUrl.includes('mongodb+srv://')) {
        console.log('   ✅ MongoDB Atlas connection string detected');
    }
    // Test 2: DNS Resolution (for SRV records)
    console.log('\n🌐 Test 2: DNS & Network Check');
    if (config_1.config.dbUrl.includes('mongodb+srv://')) {
        try {
            const url = new URL(config_1.config.dbUrl.replace('mongodb+srv://', 'https://'));
    ...
    try {
        console.log('\n   Connecting...');
        await mongoose_1.default.connect(config_1.config.dbUrl, connectionOptions);
        ...
        console.log(`   Database name: ${mongoose_1.default.connection.db?.databaseName || 'N/A'}`);
        ...
        try {
            const TestModel = mongoose_1.default.model('ConnectionTest', new mongoose_1.default.Schema({
                timestamp: Date,
                test: String,
            }));
