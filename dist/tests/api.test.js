...
async function runTests() {
    ...
    `, {}, secondToken);
        ...
        (0, test_helpers_1.assert)(bookings.data?.bookings?.[0]?.event?.date === queryEvents.data?.events?.[0]?.date, 'Nested booking event date matches the formatted public event date');
        (0, test_helpers_1.assert)(!/^\d+$/.test(bookings.data?.bookings?.[0]?.event?.date ?? ''), 'Nested booking event date is not raw epoch milliseconds');
        // Cancel booking
