...
### نمط الصياغة المعتمد في هذا المستودع

لمنع الرسائل القصيرة أو العامة، اتبع النمط التالي:

1. **عنوان واضح + محدد** بصيغة Conventional Commit
2. **Body كنقاط تبدأ بـ `-`** (سطر لكل تغيير فعلي)
3. **ابدأ كل نقطة بفعل عملي**: `Add`, `Update`, `Refine`, `Remove`, `Document`
4. **اشرح "ماذا ولماذا" باختصار** — تجنب عبارات عامة مثل "improve stuff"
5. **للتغييرات الكبيرة**: قسّم body إلى مجموعات منطقية (CI, Docs, Runtime...)

قالب مقترح:

```text
<type>(<scope>): <clear short subject>
...
# 7. فحص تكامل Docker (لازم عند تعديل Docker/Compose/CI)
node scripts/infra/validate-docker.mjs

# 8. فحص صيغة compose (اختياري لكن موصى به قبل الدفع)
docker compose config
```

**يجب أن ينجح كل ما سبق قبل الإيداع.** إيداع بأخطاء تجميع أو اختبارات فاشلة أو إعداد Docker مكسور يجب ألا يصل إلى `main`.
...
| تغيير في Docker/CI | `docs/docker-delivery.md`، `docs/tutorials/11-docker-deep-dive.md`، `README.md`، `TROUBLESHOOTING.md` |
